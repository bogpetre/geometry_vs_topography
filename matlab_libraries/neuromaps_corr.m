function [B, CI, p, effectsize, perm_var, jk_var, nu] = neuromaps_corr(DV, IV, obs_map, perm_map, varargin)
    ...
    % deal with nans in perm maps (the medial wall) through mean imputation
    new_perm_map = zeros(length(isgood),size(perm_map,2));
    parfor i = 1:size(perm_map,2)
    ...
    if ~isempty(varargin)
        for i = 1:length(varargin{1})
            % deal with confounds
            normed_arg = zscore(varargin{1}{i}');
    ...
    CI = icdf('norm', [0.025, 0.975], B, sqrt(jk_var));
...
function B = resample(samples, DV, IV, obs_map, varargin)
    ...
    rois = repmat((1:p)', 1, n-1);
    sid = repmat((1:n-1), p, 1);
