...
for i = 1:length(surface_handles)
    ...
    if doindexmap   
        ...
        if diff(sign(clim))
            ...
            whpos = c > 0;
            %     kpos = 61;   % which block of 256 colors; depends on colormap
            cpos = map_function(c(whpos), clim(1), clim(2), (kpos-1)*nvals+1, kpos*nvals); % map into indices in hot cm range of colormap
            c_colored(whpos) = cpos;
            
            whneg = c < 0;
            %     kneg = 55;   % which block of 256 colors
            cneg = map_function(c(whneg), clim(1), clim(2), (kneg-1)*nvals+1, kneg*nvals); % map into indices in cool cm range of colormap
            c_colored(whneg) = cneg;
        end
...
if any(datvec > 0) && docolorbar
    
    bar1axis = axes('Position', [.55 .55 .38 .4]);
    if doindexmap
        colormap(bar1axis, cm(2:end,:));
    else
        if diff(sign(clim)) && custom_colormap
            colormap(bar1axis, cm(1+(kpos-0.5)*nvals:kpos*nvals, :));
        else
...
if (any(datvec < 0) || clim(1) < 0) && docolorbar
    ...
    if doindexmap
        ...
        if diff(sign(clim)) && custom_colormap
            colormap(bar2axis, cm(1+(kneg-1)*nvals:(kneg-0.5)*nvals, :));
        else
    ...
    if doindexmap
        ...
        ticklabels = arrayfun(@(x1)(x1), ticklabels, 'UniformOutput', false); % make cell array
        for i = 1:length(ticklabels)
            if abs(ticklabels{i}) < 1e-7
                ticklabels{i} = '0';
            elseif abs(ticklabels{i}) < 0.01
                % scientific notation
                ticklabels{i} = sprintf('%0.1e',ticklabels{i});
            else
                % standard notation
...
lowcool = [0 .8 .8]; % [.3 .6 .9]; % max neg
hicool = [0 0 1]; % min neg, most extreme values
