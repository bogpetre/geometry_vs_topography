def block_events(subject_id, task, direction, data_dir):
    import pandas as pd
    import numpy as np
    ...
    if task == 'EMOTION':
        ...
        for i,onset in enumerate(onsets.tolist()):
            if not np.isnan(onset):
                category='Faces'
                # iterate over all elements of 'Procedure'. Will retain the final entry
                # preceeding onset. 
                for p in df['Procedure'][:i]:
                    if 'Shape' in p:
                        category = 'Shape'
                    if 'Face' in p:
                        category = 'Face'
                condition_names.append(category)
        condition_names = condition_names[::6]
        ...
        for i,onset in enumerate(onsets.tolist()):
            if not np.isnan(onset):
                names = []
                # iterate over TrialType column, resetting names each time
                # a null entry is found. That way only the last block preceeding
                # onset is saved in names
                for p in df['TrialType'][:i]:
                    if not isinstance(p, str):
                        names = []
                    else:
                        names.append(p)
                # read past the current onset until the end of the block
                for p in df['TrialType'][i:]:
                    if not isinstance(p, str):
                        break
                    else:
                ...
                # identify most frequent trial type in this block
                most_freq_cond = max(set(names), key=names.count)
                condition_names.append(most_freq_cond)
        
        
        onsets = [onset for onset in onsets.tolist() if not np.isnan(onset)]
        onsets = onsets[::8]
        condition_names = condition_names[::8]
        ...
        condition_names = [name for name in df['Type'] if name == 'Mental' or name == 'Random']
        ...
        # each row is either one or the other and all other entries are nan, so the non-nan max is 
        # whichever of the two (story or math) is present.
        onsets0 = np.nanmax([df['PresentStoryFile.OnsetTime'],
                            df['PresentMathFile.OnsetTime']], axis=0) - df['GetReady.FinishTime'][0]

        offsets0 = np.nanmax([df['PresentStoryFile.OffsetTime'],
                             df['PresentMathFile.OffsetTime']], axis=0) - df['GetReady.FinishTime'][0]

        condition_names0 = df['TaskName']
        ...
        onsets, dur, condition_names = [], [], []
        for on, off, name in zip(onsets0, offsets0, condition_names0):
            if np.isfinite(on):
                onsets.append(on)
                condition_names.append(name)
                
                # this includes response periods and ratings, match barch 2013 block designs
                # add resp_offsets to for-loop zip if you want to use this
                # dur.append((resp_offsets - on)/1000)
                
                # this does not include response periods or questions
                dur.append((off - on)/1000)
                
                # note: we divide onsets by 1000 later

        # drop final onsets since they are not not part of the scan and blow up VIFs
        onsets = onsets[:12]
        dur = dur[:12]
        condition_names = condition_names[:12]
        ...
        while i < len(onsets0):
            ...
            condition_names.append(blocktype[i])
        ...
        condition_names = [name for name in df['BlockType'] if name != 'Fixation' and isinstance(name, str)]
        # the above is kind of dumb, so let's avoid any surprises
        # length should be 10 trials/block * 10 blocks + 1 cue/block * 10 blocks = 11
        assert(len(condition_names) == 110)
        condition_names = condition_names[::11]
        ...
        condition_names = [f'{stim}-{cond}' for stim, cond in zip(df['BlockType'], df['StimType']) if isinstance(cond, str)]
        condition_names = condition_names[::10]
    ...
    dur = [[d] for d in dur]
    names = ['Task-%s-%s-%s-%02d' % (task,cond.replace(" ",""),direction,i) for i,cond in enumerate(condition_names)]
...
def hcp_events(subject_id, task, direction, data_dir):
    import pandas as pd
