...
for s = 1:height(sid_all)
    try
        tsnr1(s,:) = readmatrix(sprintf('../../derivatives/restingstate/hcp25/results/%d/tsnr.csv',sid_all.Var1(s)));
        tsnr2(s,:) = readmatrix(sprintf('../../derivatives/restingstate/hcp25/results/%d/tsnr.csv',sid_all.Var2(s)));
        
        wi_cosim1(s,:) = mean(readmatrix(sprintf('../../derivatives/restingstate/hcp25/results/%d/%s_betas/%s_similarity.csv',sid_all.Var1(s),noise,noise),'FileType','text'));
        wi_cosim2(s,:) = mean(readmatrix(sprintf('../../derivatives/restingstate/hcp25/results/%d/%s_betas/%s_similarity.csv',sid_all.Var2(s),noise,noise),'FileType','text'));        
    catch
...
for s = 1:height(sid)
    try
        wuc(s,:) = readmatrix(sprintf('../../derivatives/retest/restingstate/hcp25/bsc_retest/%s_betas/cosine/%d_v_%d_wuc.tsv',noise,sid.Var1(s),sid.Var1(s)),'FileType','text');
    catch
        warning('Could not import pair %d', s);
    end
end
...
for s = 1:height(sid)
    try
        cosim(s,:) = mean(dlmread(sprintf('../../derivatives/retest/restingstate/hcp25/bsc_retest/%s_betas/cosine/%d_v_%d_cosim.tsv', noise, sid.Var1(s), sid.Var1(s)), '\t'));
    catch
        warning('Could not import pair %d', s);
    end
end
...
switch noise
    case 'whitened'
        T = {'Mean RSN tSNR','across runs and participants',['(Whitened \beta, N = ', sprintf('%0d)',size(tsnr,1))]};
    case 'standardized'
        T = {'Mean RSN tSNR','across runs and participants',['(Standardized \beta, N = ', sprintf('%0d)',size(tsnr,1))]};
end
plot_to_brain(B,1:length(B),cmaprange,T,fs+2);
...
switch noise
    case 'whitened'
        T = {'RSN Topoography Test-Retest Reliability',['(Whitened \beta, cos\theta, ', sprintf('N=%d)',size(cosim,1))]};
    case 'standardized'
        T = {'RSN Topoography Test-Retest Reliability',['(t-stat, cos\theta, ', sprintf('N=%d)',size(cosim,1))]};
end
plot_to_brain(B,1:length(B),cmaprange,T,fs+2);
...
title(['Mean r^2 = ',sprintf('%0.3f',B(1)^2)]);
...
B = mean_xsubject_B(wuc,wi_cosim(sid_all_in_retest,:),'std');
title(['Mean r^2 = ',sprintf('%0.3f',B(1)^2)]);
...
B = mean_xsubject_B(wuc,cosim,'std');
title(['Mean r^2 = ',sprintf('%0.3f',B(1)^2)]);
...
exportgraphics(gcf,sprintf('panels_%s/test_retest_scatterplots.png',noise),'ContentType','image','Resolution',300);
